...
def build_login_copy(admin_count: int) -> dict[str, str]:
    auto_create_enabled = is_true_env("EASY2_GOOGLE_AUTO_CREATE", default=True)

    if admin_count <= 0 and auto_create_enabled:
        return {
            "title": "Crear la primera cuenta o iniciar sesión",
            "lead": (
                "Entra con Google para crear la primera cuenta de administrador "
                "o acceder si ya te habían dado acceso."
            ),
            "button": "Crear cuenta o iniciar sesión con Google",
            "helper": (
                "Si tu correo está permitido, tu cuenta se creará automáticamente "
    ...
    if auto_create_enabled:
        return {
            ...
            "lead": (
                "Usa tu cuenta de Google para entrar al panel. "
                "Si tu correo está autorizado, también podrás crear tu propia cuenta."
            ),
            "button": "Continuar con Google",
            "helper": (
                "Si ya tienes cuenta, entrarás directamente. "
                "Si no la tienes y tu correo está permitido, se creará al momento."
            ),
        }
    ...
    return {
        ...
        "lead": (
            "Usa tu cuenta de Google para entrar al panel de administrador."
        ),
        "button": "Iniciar sesión con Google",
        "helper": (
            "La creación automática de cuentas está desactivada. "
            "Necesitas una cuenta ya autorizada."
        ),
    }
...
def login() -> Any:
    ...
    return render_template(
        ...
        login_copy=build_login_copy(admin_count),
